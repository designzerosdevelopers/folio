...
<html lang="en-US">
  <head>
    ...
    <link href="{{ asset('portfolio_assets/super/css/bootstrap-icons/bootstrap-icons.css?ver=1.2.0')}}" rel="stylesheet">
...
<header>
  <div class="cover bg-light">
    <div class="container px-3">
      <div class="row">
        <div class="col-lg-6 p-2"><img class="img-fluid" src="{{ asset('portfolio_assets/super/images/illustrations/hello3.svg')}}" alt="hello"/></div>
        <div class="col-lg-6">
          <div class="mt-5">
            <p class="lead text-uppercase mb-1">Hello!</p>
            <h1 class="intro-title marker">I’m</h1> <h1 class="intro-title marker" data-aos="fade-left" id="user-name" data-aos-delay="50">{{ !empty($user->name) ? $user->name : 'Walter Patterson' }}</h1>
            <p class="lead fw-normal mt-3" data-aos="fade-up" id="user-profession" data-aos-delay="100">{{ !empty($user->profession) ? $user->profession : 'Web Developer & Mobile Application Developer' }}</p>
            <div class="social-nav" data-aos="fade-up" data-aos-delay="200">
              <nav role="navigation">
                <ul class="nav justify-content-left">
                  @if(!empty($user->twitter))<li class="nav-item"><a class="nav-link" href="https://{{$user->twitter}}" title="Twitter"><i class="fab fa-twitter"></i><span class="menu-title sr-only">Twitter</span></a></li>@endif
                  @if(!empty($user->facebook))<li class="nav-item"><a class="nav-link" href="https://{{$user->facebook}}" title="Facebook"><i class="fab fa-facebook"></i><span class="menu-title sr-only">Facebook</span></a></li>@endif
                  @if(!empty($user->instagram))<li class="nav-item"><a class="nav-link" href="https://{{$user->instagram}}" title="Instagram"><i class="fab fa-instagram"></i><span class="menu-title sr-only">Instagram</span></a></li>@endif
                  @if(!empty($user->linked_in))<li class="nav-item"><a class="nav-link" href="https://{{$user->linked_in}}" title="LinkedIn"><i class="fab fa-linkedin"></i><span class="menu-title sr-only">LinkedIn</span></a></li>@endif
                  @if(!empty($user->github))<li class="nav-item"><a class="nav-link" href="https://{{$user->github}}" title="GitHub"><i class="fab fa-github"></i><span class="menu-title sr-only">GitHub</span></a></li>@endif
                  @if(!empty($user->upwork))<li class="nav-item"><a class="nav-link" href="https://{{$user->upwork}}" title="Upwork"><i class="fa-brands fa-upwork"></i><span class="menu-title sr-only">Upwork</span></a></li>@endif
                  @if(!empty($user->freelancer))<li class="nav-item"><a class="nav-link" href="https://{{$user->freelancer}}" title="Freelancer"><i class="fab fa-freelancer"></i><span class="menu-title sr-only">Freelancer</span></a></li>@endif
                  @if(!empty($user->youtube))<li class="nav-item"><a class="nav-link" href="https://{{$user->youtube}}" title="Youtube"><i class="fab fa-youtube"></i><span class="menu-title sr-only">Youtube</span></a></li>@endif
                </ul>
              </nav>
            </div>
  ...
  <div class="container-narrow">
    <div class="row">
      <div class="col-md-6">
        ...
        <p id="user-description">@if(!empty($user->profile_description)) {{$user->profile_description}} @else I am passionate about UI/UX design and Web Design. I am a skilled front-end developer and master of graphic design tools such as Photoshop and Sketch. I am a quick learner and a team worker that gets the job done. I can easily capitalize on low hanging fruits and quickly maximize timely deliverables for real-time schemas.@endif</p>
        <div class="row mt-3">
          <div class="col-sm-2">
            <div class="pb-1">Age:</div>
          </div>
          <div class="col-sm-10">
            <div class="pb-1 fw-bolder" id="user-age">{{!empty($user->age)? $user->age :'28'}} </div>
          </div>
          <div class="col-sm-2">
            <div class="pb-1">Email:</div>
          </div>
          <div class="col-sm-10">
            <div class="pb-1 fw-bolder" id="user-email">{{!empty($user->email)? $user->email :'company@company.com'}}</div>
          </div>
          <div class="col-sm-2">
            <div class="pb-1">Phone:</div>
          </div>
          <div class="col-sm-10">
            <div class="pb-1 fw-bolder" id="contact-number">{{!empty($user->contact_number)? $user->contact_number :'+0718-111-0011'}}</div>
          </div>
          <div class="col-sm-2">
            <div class="pb-1">Address:</div>
          </div>
          <div class="col-sm-10">
            <div class="pb-1 fw-bolder" id="user-address">{{!empty($user->address)? $user->address :'New York, USA'}}</div>
          </div>
          <div class="col-sm-2">
            <div class="pb-1">Gender</div>
          </div>
          <div class="col-sm-10">
            <div class="pb-1 fw-bolder" id="user-gender">{{!empty($user->gender)? $user->gender :'Male'}}</div>
          </div>
        </div>
      </div>
      <div class="col-md-5 offset-md-1" data-aos="fade-left" data-aos-delay="100"><img class="avatar img-fluid mt-2" src="{{ asset('profile_pic/' . (!empty($user->image) ? $user->image : 'noimage.jpg')) }}" width="400" height="400" alt="Profile photo"/></div>
    </div>
  </div>
</div>
    ...
    {{-- <div class="text-center">
      ...
      <div class="row py-3 justify-content-center">
        @if(!empty($services))
          ...
          @endforeach
        @else
        <div class="col-md-3 text-center" data-aos="fade-up" data-aos-delay="100"><img class="mb-2" src="{{asset('portfolio_assets/super/images/services/web-design.svg')}}" width="96" height="96" alt="web design"/>
          <div class="h5">Web Design</div>
        </div>
        <div class="col-md-3 text-center" data-aos="fade-up" data-aos-delay="200"><img class="mb-2" src="{{asset('portfolio_assets/super/images/services/graphic-design.svg')}}" width="96" height="96" alt="graphic design"/>
          <div class="h5">Graphic Design</div>
        </div>
        <div class="col-md-3 text-center" data-aos="fade-up" data-aos-delay="300"><img class="mb-2" src="{{asset('portfolio_assets/super/images/services/ui-ux.svg')}}" width="96" height="96" alt="ui-ux"/>
          <div class="h5">UI/UX</div>
        </div>
        <div class="col-md-3 text-center" data-aos="fade-up" data-aos-delay="400"><img class="mb-2" src="{{asset('portfolio_assets/super/images/services/app-development.svg')}}" width="96" height="96" alt="app development"/>
    ...
    <div class="bg-light p-3">
      <div class="row justify-content-center">
        ...
        @foreach($skills as $skill)
          <div class="py-1">
            <div class="d-flex text-small fw-bolder"><span class="me-auto">{{$skill->skill_name}}</span><span>{{$skill->skill_percentage}}%</span></div>
            <div class="progress my-1">
              <div class="progress-bar bg-primary" role="progressbar" data-aos="zoom-in-right" data-aos-delay="100" data-aos-anchor=".skills-section" style="width: {{$skill->skill_percentage}}%" aria-valuenow="{{$skill->skill_percentage}}" aria-valuemin="0" aria-valuemax="100"></div>
            </div>
          </div>
        @endforeach
        </div>
...
<div class="section px-2 px-lg-4 pb-4 pt-5 mb-5" id="contact">
  <div class="container-narrow">
    ...
    <div class="form-group my-2">
      ...
      <input class="form-control" type="email" id="fc-nameDisplay" name="_replyto" required>
    </div>
  <div class="form-group my-2">
    ...
    <script>
      ...
      window.addEventListener('message', function(event) {
          ...
          if (dataValue) {
            ...
            const centerPosition = element.offsetTop - (window.innerHeight / 2) + (element.clientHeight / 2);
        ...
        window.addEventListener('message', function (event) {
          if (typeof event.data === 'object') {
            ...
            updateAndScroll(document.getElementById('user-profession'), 'profession', receivedData.profession);
            updateAndScroll(document.getElementById('user-description'), 'description', receivedData.description);
            updateAndScroll(document.getElementById('user-email'), 'email', receivedData.email);
            updateAndScroll(document.getElementById('contact-number'), 'contact', receivedData.contact);
            updateAndScroll(document.getElementById('user-age'), 'age', receivedData.age);
            updateAndScroll(document.getElementById('user-address'), 'address', receivedData.address);
